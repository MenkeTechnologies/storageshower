//! `scan_directory` with one hundred thirty-two files (largest first).
...
#[test]
fn one_hundred_thirty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 132);
    assert_eq!(v[0].name, "n131");
    assert!(v[0].size >= v[131].size);
}
