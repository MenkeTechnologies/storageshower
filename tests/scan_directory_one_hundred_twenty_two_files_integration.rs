//! `scan_directory` with one hundred twenty two files (largest first).
...
#[test]
fn one_hundred_twenty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 122);
    assert_eq!(v[0].name, "n121");
    assert!(v[0].size >= v[121].size);
}
