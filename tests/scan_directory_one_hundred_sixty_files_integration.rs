//! `scan_directory` with one hundred sixty files (largest first).
...
#[test]
fn one_hundred_sixty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 160);
    assert_eq!(v[0].name, "n159");
    assert!(v[0].size >= v[159].size);
}
