//! `scan_directory` with one hundred forty-nine files (largest first).
...
#[test]
fn one_hundred_forty_nine_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 149);
    assert_eq!(v[0].name, "n148");
    assert!(v[0].size >= v[148].size);
}
