//! `scan_directory` with one hundred forty files (largest first).
...
#[test]
fn one_hundred_forty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 140);
    assert_eq!(v[0].name, "n139");
    assert!(v[0].size >= v[139].size);
}
