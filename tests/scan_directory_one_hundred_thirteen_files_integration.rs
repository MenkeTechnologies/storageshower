//! `scan_directory` with one hundred thirteen files (largest first).
...
#[test]
fn one_hundred_thirteen_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 113);
    assert_eq!(v[0].name, "n112");
    assert!(v[0].size >= v[112].size);
}
