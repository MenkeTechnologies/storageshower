//! `scan_directory` with one hundred eighty-six files (largest first).
...
#[test]
fn one_hundred_eighty_six_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 186);
    assert_eq!(v[0].name, "n185");
    assert!(v[0].size >= v[185].size);
}
