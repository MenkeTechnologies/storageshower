//! `scan_directory` with one hundred forty-two files (largest first).
...
#[test]
fn one_hundred_forty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 142);
    assert_eq!(v[0].name, "n141");
    assert!(v[0].size >= v[141].size);
}
