//! `scan_directory` with one hundred seventeen files (largest first).
...
#[test]
fn one_hundred_seventeen_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 117);
    assert_eq!(v[0].name, "n116");
    assert!(v[0].size >= v[116].size);
}
