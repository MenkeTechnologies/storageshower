//! `scan_directory` with one hundred eighteen files (largest first).
...
#[test]
fn one_hundred_eighteen_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 118);
    assert_eq!(v[0].name, "n117");
    assert!(v[0].size >= v[117].size);
}
