//! `SortMode::Name` with two bookmarks among three mounts.
...
#[test]
fn two_bookmarks_before_non_bookmark_name_asc() {
    let mut app = app_with(vec![disk("/a"), disk("/m"), disk("/x")]);
    app.prefs.sort_mode = SortMode::Name;
    app.prefs.bookmarks = vec!["/m".into(), "/x".into()];
    app.update_sorted();
    let m: Vec<_> = app
        .sorted_disks()
        .iter()
        ...
        .collect();
    assert_eq!(m, vec!["/m", "/x", "/a"]);
}

#[test]
fn both_bookmarks_only_name_asc() {
    let mut app = app_with(vec![disk("/z"), disk("/a")]);
    app.prefs.sort_mode = SortMode::Name;
    app.prefs.bookmarks = vec!["/z".into(), "/a".into()];
    app.update_sorted();
