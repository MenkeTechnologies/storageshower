//! `scan_directory` with one hundred fifty-nine files (largest first).
...
#[test]
fn one_hundred_fifty_nine_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 159);
    assert_eq!(v[0].name, "n158");
    assert!(v[0].size >= v[158].size);
}
