//! `scan_directory` with one hundred thirty-one files (largest first).
...
#[test]
fn one_hundred_thirty_one_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 131);
    assert_eq!(v[0].name, "n130");
    assert!(v[0].size >= v[130].size);
}
