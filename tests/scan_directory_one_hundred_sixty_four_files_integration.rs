//! `scan_directory` with one hundred sixty-four files (largest first).
...
#[test]
fn one_hundred_sixty_four_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 164);
    assert_eq!(v[0].name, "n163");
    assert!(v[0].size >= v[163].size);
}
