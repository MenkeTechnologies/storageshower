//! `scan_directory` with one hundred fifty-two files (largest first).
...
#[test]
fn one_hundred_fifty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 152);
    assert_eq!(v[0].name, "n151");
    assert!(v[0].size >= v[151].size);
}
