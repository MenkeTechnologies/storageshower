//! `scan_directory` with one hundred thirty files (largest first).
...
#[test]
fn one_hundred_thirty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 130);
    assert_eq!(v[0].name, "n129");
    assert!(v[0].size >= v[129].size);
}
