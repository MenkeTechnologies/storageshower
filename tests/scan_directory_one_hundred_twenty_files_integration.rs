//! `scan_directory` with one hundred twenty files (largest first).
...
#[test]
fn one_hundred_twenty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 120);
    assert_eq!(v[0].name, "n119");
    assert!(v[0].size >= v[119].size);
}
