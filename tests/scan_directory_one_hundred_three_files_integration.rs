//! `scan_directory` with one hundred three files (largest first).
...
#[test]
fn one_hundred_three_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 103);
    assert_eq!(v[0].name, "n102");
    assert!(v[0].size >= v[102].size);
}
