//! `scan_directory` with one hundred sixty-nine files (largest first).
...
#[test]
fn one_hundred_sixty_nine_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 169);
    assert_eq!(v[0].name, "n168");
    assert!(v[0].size >= v[168].size);
}
