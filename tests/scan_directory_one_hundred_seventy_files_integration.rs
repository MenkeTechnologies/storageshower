//! `scan_directory` with one hundred seventy files (largest first).
...
#[test]
fn one_hundred_seventy_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 170);
    assert_eq!(v[0].name, "n169");
    assert!(v[0].size >= v[169].size);
}
