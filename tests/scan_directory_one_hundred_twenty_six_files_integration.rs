//! `scan_directory` with one hundred twenty six files (largest first).
...
#[test]
fn one_hundred_twenty_six_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 126);
    assert_eq!(v[0].name, "n125");
    assert!(v[0].size >= v[125].size);
}
