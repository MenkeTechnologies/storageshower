//! `scan_directory` with one hundred eleven files (largest first).
...
#[test]
fn one_hundred_eleven_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 111);
    assert_eq!(v[0].name, "n110");
    assert!(v[0].size >= v[110].size);
}
