//! `scan_directory` with one hundred sixty-two files (largest first).
...
#[test]
fn one_hundred_sixty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 162);
    assert_eq!(v[0].name, "n161");
    assert!(v[0].size >= v[161].size);
}
