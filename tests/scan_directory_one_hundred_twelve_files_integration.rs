//! `scan_directory` with one hundred twelve files (largest first).
...
#[test]
fn one_hundred_twelve_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 112);
    assert_eq!(v[0].name, "n111");
    assert!(v[0].size >= v[111].size);
}
