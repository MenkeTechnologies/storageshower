//! `scan_directory` with one hundred eighty-two files (largest first).
...
#[test]
fn one_hundred_eighty_two_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 182);
    assert_eq!(v[0].name, "n181");
    assert!(v[0].size >= v[181].size);
}
