//! `scan_directory` with one hundred eight files (largest first).
...
#[test]
fn one_hundred_eight_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 108);
    assert_eq!(v[0].name, "n107");
    assert!(v[0].size >= v[107].size);
}
