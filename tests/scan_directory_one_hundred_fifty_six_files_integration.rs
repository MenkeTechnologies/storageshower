//! `scan_directory` with one hundred fifty-six files (largest first).
...
#[test]
fn one_hundred_fifty_six_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 156);
    assert_eq!(v[0].name, "n155");
    assert!(v[0].size >= v[155].size);
}
