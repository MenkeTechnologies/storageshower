//! `scan_directory` with one hundred fifty-four files (largest first).
...
#[test]
fn one_hundred_fifty_four_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 154);
    assert_eq!(v[0].name, "n153");
    assert!(v[0].size >= v[153].size);
}
