//! `scan_directory` with one hundred forty-five files (largest first).
...
#[test]
fn one_hundred_forty_five_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 145);
    assert_eq!(v[0].name, "n144");
    assert!(v[0].size >= v[144].size);
}
