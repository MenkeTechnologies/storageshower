//! `scan_directory` with one hundred fifty-one files (largest first).
...
#[test]
fn one_hundred_fifty_one_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 151);
    assert_eq!(v[0].name, "n150");
    assert!(v[0].size >= v[150].size);
}
