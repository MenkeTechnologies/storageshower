//! `scan_directory` with one hundred eighty-one files (largest first).
...
#[test]
fn one_hundred_eighty_one_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 181);
    assert_eq!(v[0].name, "n180");
    assert!(v[0].size >= v[180].size);
}
