//! `scan_directory` with one hundred eighty files (largest first).
...
#[test]
fn one_hundred_eighty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 180);
    assert_eq!(v[0].name, "n179");
    assert!(v[0].size >= v[179].size);
}
