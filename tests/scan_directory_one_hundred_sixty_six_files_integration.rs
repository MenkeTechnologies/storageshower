//! `scan_directory` with one hundred sixty-six files (largest first).
...
#[test]
fn one_hundred_sixty_six_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 166);
    assert_eq!(v[0].name, "n165");
    assert!(v[0].size >= v[165].size);
}
