//! `scan_directory` with one hundred fifty files (largest first).
...
#[test]
fn one_hundred_fifty_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 150);
    assert_eq!(v[0].name, "n149");
    assert!(v[0].size >= v[149].size);
}
