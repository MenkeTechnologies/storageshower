//! `scan_directory` with one hundred fourteen files (largest first).
...
#[test]
fn one_hundred_fourteen_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 114);
    assert_eq!(v[0].name, "n113");
    assert!(v[0].size >= v[113].size);
}
