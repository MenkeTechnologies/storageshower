//! `scan_directory` with one hundred sixty-five files (largest first).
...
#[test]
fn one_hundred_sixty_five_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 165);
    assert_eq!(v[0].name, "n164");
    assert!(v[0].size >= v[164].size);
}
