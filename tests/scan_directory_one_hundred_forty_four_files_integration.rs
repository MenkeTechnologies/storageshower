//! `scan_directory` with one hundred forty-four files (largest first).
...
#[test]
fn one_hundred_forty_four_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 144);
    assert_eq!(v[0].name, "n143");
    assert!(v[0].size >= v[143].size);
}
