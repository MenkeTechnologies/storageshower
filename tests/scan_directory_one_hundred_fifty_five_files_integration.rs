//! `scan_directory` with one hundred fifty-five files (largest first).
...
#[test]
fn one_hundred_fifty_five_files_order_by_size_desc() {
    ...
    assert_eq!(v.len(), 155);
    assert_eq!(v[0].name, "n154");
    assert!(v[0].size >= v[154].size);
}
